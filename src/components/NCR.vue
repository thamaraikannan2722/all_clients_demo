<template>
  <div id="ncr">
    <div
      v-if="
        (ncrbankdetails && ncrbankdetails.length) ||
    ...
    >
      <div class="flex -mt-12 ml-200">
        <button @click="exportExcel" class="ml-2">
          <div
            class="flex flex-2 gap-1 justify-center content-center group-group4 px-2 p-1 border-2 rounded border-black hover:bg-red-600 shadow-xl"
          >
            <h3
              class="group-group4-hover:text-white transition ease-out duration-300"
            >
              Download -
            </h3>
            <i
              class="fas fa-download mt-1 group-group4-hover:text-white transition ease-out duration-300"
            ></i>
          </div>
        </button>
      </div>
      ...
      <div class="grid grid-cols-5 gap-10 ml-10 mr-10 mt-5 mb-5">
        <div
          v-for="(ncr, id) in ncrbankdetails"
          :key="id"
        ...
        >
          <div class="group">
            <div
              class="hidden group-hover:block group-hover:animate-slideup1 -mt-24"
            >
              <div class="flex gap-5 justify-center content-center mb-6">
                <div>
                  ...
                  >
                    <div
                      class="grid grid-cols-1 justify-items-center content-center ml-2"
                    >
                      <i
                        class="fas fa-dot-circle text-orange-700 p-auto justify-center content-center"
                      ></i>
                    </div>
                  ...
                  <h2 class="animate-slideup1 text-sm truncate">
                    Opened Tickets
                  </h2>
                </div>
                <div>
                  ...
                  >
                    ...
                    >
                      <i
                        class="fas fa-dot-circle text-yellow-700 p-auto justify-center content-center"
                      ></i>
                    </div>
                    <h2 class="text-lg self-center font-semibold ml-1 mr-2">
                      {{ ncr.closedTickets }}
            ...
            <h2 class="text-lg">{{ ncr.name }}</h2>
            <h2 class="text-5xl font-semibold">{{ ncr.totalSite }}</h2>
            <div class="flex gap-5 justify-center content-center">
              <div
                class="flex justify-center content-center mt-4 mb-2 -pt-3 rounded-full"
              >
                <h2 class="text-md self-center ml-1 mr-2">Active Sites</h2>
              </div>
              <div
              ...
              >
                <button
                  type="button"
                  :value="ncr.name"
                  v-on:click="LiveSites($event, ncr.name)"
                >
                  <div>
            ...
            <div
              class="hidden group-hover:block group-hover:animate-slidebottom"
            >
              <div class="flex gap-5 justify-center content-center mt-3">
                <div
                  class="flex justify-center content-center mt-4 mb-2 -pt-3 rounded-full"
                >
                  <h2 class="text-md self-center ml-1 mr-2">NC-30</h2>
                </div>
                <div
                ...
                >
                  <button
                    :value="ncr.name"
                    v-on:click="NC30Sites($event, ncr.name)"
                  >
                    <div>
                      <div class="flex justify-center items-center ml-2">
        ...
        <div
          class="container col-span-2 rounded-xl overflow-y-scroll border border-black mt-12"
        >
            ...
            <table class="table-auto border border-collapse text-sm -mx-24">
              ...
              <tbody class="divide-y divide-gray-400 text-center">
                <tr
                  ...
                  v-for="mail in filterMailsByBank"
                  :key="mail.id"
                  @change="filterMailsByBank"
                >
                  <td class="text-xs">{{ mail.bank }}</td>
                  <td class="text-xs">{{ mail.subject }}</td>
                  <td class="text-xs">{{ mail.atTime }}</td>
                  <td
                    v-if="mail.status === 'Mail Sent'"
                    class="border border-b-2 border-black bg-green-400 font-semibold text-xs"
                  >
                    {{ mail.status }}
                  </td>
                  <td
                    v-else-if="mail.status === 'Mail Not Sent'"
                    class="border border-b-2 border-black bg-red-400 font-semibold text-xs"
                  >
                    {{ mail.status }}
                  </td>
                  <td
                    v-if="mail.status === 'File Missing'"
                    class="border border-b-2 border-black bg-orange-400 font-semibold text-xs"
                  >
                    {{ mail.status }}
                  </td>
                </tr>
...
export default {
  ...
  created() {
    ...
    this.filterBanks();
    this.intervalFetchBankData();
    this.intervalFetchReportsData();
    //this.loggedInUser();
    //this.forEachBanks();
    ...
    console.log(this.ncrbankdetails);
  ...
  methods: {
    ...
    intervalFetchBankData: function () {
      setInterval(
        function () {
          this.fetchBanksData();
        }.bind(this),
    ...
    intervalFetchReportsData: function () {
      setInterval(
        function () {
          this.fetchReportsData();
        }.bind(this),
    ...
    LiveSites: function (e, id) {
      ...
      //console.log(this.bankName)
      //location.href='';
      this.$router.push("/ncrbanklivesites");
    },
    NC30Sites: function (e, id1) {
      this.$store.commit("setNC30Sites", { onClickNC30Sites: id1 });
      this.$router.push("/ncrbanknc30sites");
    },
    fetchBanksData: function () {
      var vm = this;
      var url3 = "./json/ncrbankdetails.json";
      vm.ncrbankdetails.slice();
      vm.ncrbankdetails.length = 0;
      axios
        .get(url3)
        .then(function (response1) {
          response1.data.ncr.forEach((banks) => {
            vm.ncrbankdetails.push(banks);
    ...
    fetchReportsData: function () {
      var vm = this;
      var url4 = "./json/ncrbankdetails.json";
      vm.reports.slice();
      vm.reports.length=0;
      axios
        .get(url4)
        .then(function (response2) {
          response2.data.ncr.forEach((banks) => {
            banks.reports.forEach((email) => {
    ...
    filterBanks: function () {
      var vm = this;
      var url7 = "./json/ncrbankdetails.json";
