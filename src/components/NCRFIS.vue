    ...
    <div class="flex justify-center gap-58 mt-4">
      <div>
        <h2 class="text-xl text-blue-600 text-center font-bold mt-4">
          NCR-FIS Bank Details
        </h2>
      </div>
    </div>
    <div class="grid grid-cols-5 gap-10 ml-10 mr-10 mt-5 mb-5">
      <div
        v-for="(ncrfis, id) in ncrfisbankdetails"
        :key="id"
      ...
      <div class="hidden group-hover:block group-hover:animate-slideup1 -mt-24">
        ...
        <h2 class="text-lg">{{ ncrfis.name }}</h2>
        <h2 class="text-5xl font-semibold">{{ ncrfis.totalSite }}</h2>
        <div class="flex gap-5 justify-center content-center">
          ...
          >
          <button type="button" :value="ncrfis.name" v-on:click="LiveSites($event, ncrfis.name)">
            <div>
        ...
        <div class="flex gap-5 justify-center content-center mt-3">
          ...
          >
            <button :value="ncrfis.name" v-on:click="NC30Sites($event, ncrfis.name)">
            <div>
            <div class="flex justify-center items-center ml-2">
...
export default {
  ...
  created() {
    ...
    console.log(this.ncrfisbankdetails);
  ...
  methods: {
    exportExcel () {
      saveExcel({
        ...
        fileName:"NCRFIS_Mail_Status.xlsx",
        columns: [
    ...
    LiveSites: function (e, id) {
      ...
      //console.log(this.bankName)
      //location.href='';
      this.$router.push('/ncrfisbanklivesites')
    },
    NC30Sites: function (e, id1) {
      this.$store.commit('setNC30Sites', {onClickNC30Sites: id1});
      this.$router.push('/ncrfisbanknc30sites')
    },
    fetchBanksData: function () {
      var vm = this;
      var url3 =
        "./json/ncrfisbankdetails.json";
      //vm.emailreports.slice();
      //vm.emailreports.length=0;
      axios
        .get(url3)
        .then(function (response1) {
          response1.data.ncrfis.forEach((banks) => {
            vm.ncrfisbankdetails.push(banks);
