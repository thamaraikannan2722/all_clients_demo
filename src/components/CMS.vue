<template>
  <div id="cms">
    ...
    <div class="grid grid-cols-5 gap-10 ml-10 mr-10 mt-5 mb-5">
      <div
        v-for="(cms, id) in cmsbankdetails"
        :key="id"
      ...
      <div class="hidden group-hover:block group-hover:animate-slideup1 -mt-24">
        ...
        <h2 class="text-lg">{{ cms.name }}</h2>
        <h2 class="text-5xl font-semibold">{{ cms.totalSite }}</h2>
        <div class="flex gap-5 justify-center content-center">
          ...
          >
          <button type="button" :value="cms.name" v-on:click="LiveSites($event, cms.name)">
            <div>
        ...
        <div class="flex gap-5 justify-center content-center mt-3">
          ...
          >
            <button :value="cms.name" v-on:click="NC30Sites($event, cms.name)">
            <div>
            <div class="flex justify-center items-center ml-2">
...
export default {
  ...
  created() {
    ...
    console.log(this.cmsbankdetails);
  ...
  methods: {
    ...
    LiveSites: function (e, id) {
      ...
      //console.log(this.bankName)
      //location.href='';
      this.$router.push('/cmsbanklivesites')
    },
    NC30Sites: function (e, id1) {
      this.$store.commit('setNC30Sites', {onClickNC30Sites: id1});
      this.$router.push('/cmsbanknc30sites')
    },
    fetchBanksData: function () {
      var vm = this;
      var url3 =
        "./json/cmsbankdetails.json";
      //vm.emailreports.slice();
      //vm.emailreports.length=0;
      axios
        .get(url3)
        .then(function (response1) {
          response1.data.cms.forEach((banks) => {
            vm.cmsbankdetails.push(banks);
    ...
    filterBanks: function () {
      var vm = this;
      var url7 = "./json/cmsbankdetails.json";
      //vm.emailreports.slice();
      //vm.emailreports.length=0;
      axios
        .get(url7)
        .then(function (response2) {
          response2.data.cms.forEach((banks) => {
              vm.banks.push(banks);
